...
const PRODUCTION_PATTERNS: RegExp[] = [
  /^https:\/\/(.*\.)?rudraas\.in$/,
  /^https:\/\/rudraas-intel-[a-z0-9-]+\.vercel\.app$/,
...
export function getCorsHeaders(req: Request): Record<string, string> {
  ...
  const allowOrigin = isAllowedOrigin(origin) ? origin : 'https://rudraas.in';
  return {
    'Access-Control-Allow-Origin': allowOrigin,
    'Access-Control-Allow-Methods': 'GET, POST, OPTIONS',
    'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-RUDRAAS-Key',
