...
export function exportToJSON(data: ExportData, filename = 'rudraas-intel-export'): void {
  const jsonStr = JSON.stringify(data, null, 2);
  downloadFile(jsonStr, `${filename}.json`, 'application/json');
}

export function exportToCSV(data: ExportData, filename = 'rudraas-intel-export'): void {
  const lines: string[] = [];
...
export class ExportPanel {
  ...
  private export(format: ExportFormat): void {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const filename = `rudraas-intel-${timestamp}`;
